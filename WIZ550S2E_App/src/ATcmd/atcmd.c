...
void atc_run(void)
{
	...
	if(UART_read(&recv_char, 1) <= 0) return; // ?ÖÎ†• Í∞??ÜÎäî Í≤ΩÏö∞		printf("RECV: 0x%x\r\n", recv_char);

	if(atci.sendsock != VAL_NONE)
	{
		atci.sendbuf[atci.worklen++] = recv_char;
		if(atci.worklen >= atci.sendlen) { // ?ÖÎ†•???ÑÎ£å?òÎ©¥
	...
	if(isgraph(recv_char) == 0 && (recv_char != 0x20))	// ?úÏñ¥ Î¨∏Ïûê Ï≤òÎ¶¨
	{	//printf("ctrl\r\n");
		switch(recv_char) {
			case 0x0d:	// CR(\r)
				break;	//		do nothing
	...
	else if(buflen < ATCMD_BUF_SIZE-1)		// -1 ?¥Ïú† : 0 ???òÎÇò ?ÑÏöî?òÎ?Î°?
	{
...
static void cmd_set_prev(uint8_t buflen)
{
	...
	if(prevbuf[previdx] == NULL) CRITICAL_ERR("malloc fail");	//  ÎßåÏïΩ ?§Ìå®?¥ÎèÑ Í±??òÍ≥† ?∂ÏúºÎ©??òÏ†ï
	else {
...
{ \
	...
	DBGA("Debug: (%s)", tmpptr);	//ÏµúÎ? arg?òÍ≤å ?§Ïñ¥??Í≤??ïÏù∏??- Strict Param ?ïÏ±Ö
...
static void hdl_nset(void)
{
	...
	if(atci.tcmd.sign == CMD_SIGN_NONE) atci.tcmd.sign = CMD_SIGN_QUEST;	// x???Î°?ÏπòÌôò
	if(atci.tcmd.sign == CMD_SIGN_QUEST)
	{
...
static void hdl_nopen(void)
{
	...
	else if(atci.tcmd.sign == CMD_SIGN_EQUAL)
	{
		...
		} else {	// 'A'	Î¨¥Ïãú?ïÏ±Ö?¥ÎÉê ?ÑÎãàÎ©??ÑÎ? ?ïÏù∏ ?ïÏ±Ö?¥ÎÉê
...
static void hdl_nsend(void)
{
	...
	else if(atci.tcmd.sign == CMD_SIGN_EQUAL)
	{
		...
		atci.sendsock = num;	// ?†Ìö®??Í≤Ä?¨Í? ?ÑÎ£å?òÎ©¥ SENDÎ™®ÎìúÎ°??ÑÌôò
		atci.worklen = 0;
...
static void hdl_mset(void)
{
	...
	if(atci.tcmd.sign == CMD_SIGN_NONE) atci.tcmd.sign = CMD_SIGN_QUEST;	// [?] Íµ¨ÌòÑ
	if(atci.tcmd.sign == CMD_SIGN_QUEST)
	{
		if(atci.tcmd.arg1[0] != 0) {
			if(str_check(isdigit, atci.tcmd.arg1) != RET_OK) RESP_CDR(RET_WRONG_ARG, 1);
	...
	else if(atci.tcmd.sign == CMD_SIGN_INDIV)
	{
		if(atci.tcmd.arg1[0] != 0) {
			...
			if(num == 1) {
				...
				CMD_CLEAR();
				act_mset_a(0, poll, 0);
			} else RESP_CDR(RET_NOT_ALLOWED, 2);	// Íµ?? ?§Ï†ï ?ÑÏßÅ Íµ¨ÌòÑ?àÌï®
		} else RESP_CDR(RET_WRONG_ARG, 1);
	}
	else if(atci.tcmd.sign == CMD_SIGN_EQUAL)
	{
